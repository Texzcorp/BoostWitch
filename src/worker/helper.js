...
export const ui = {
  ...
  focusScroll: (parentElement) => {
    const isScrolledToBottom = parentElement.scrollHeight - parentElement.clientHeight <= parentElement.scrollTop + 100;
  ...
  _createLog: (type, targetArea, message, channel = '') => {
    ...
    switch (type) {
      case 'chat':
        li.innerHTML = `<span class="l-time">${general.getTime(false)}</span><b> > </b><span class="l-channel">${channel}</span><b> / </b>`;
