  ...
  try {
    ...
    switch (mode) {
      case 'auto': {
        ...
        const response = await tw.apiRequest('https://api.twitch.tv/helix/categories', `query=${usernames}`);
    ...
    general.stopIf(isOff);
    ui.logStatus(`Working on ${finalUsers.length} user(s) and ${finalChannels.length} channel(s) . . .`);
    ...
    ws.onopen = async () => {
      ...
      ui.logStatus('To see the chat logs, go to the <a href="#watch">Watch</a> tab.');
      ...
      ws.onmessage = (message) => {
        ...
        if (msgData) {
          if (msgData.command === 'PRIVMSG' && selectedUsers.has(msgData.user)) {
            ui.logChat(msgData);
